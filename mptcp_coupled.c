...
static int diff_interval = 1000;//threshold=1000ms

struct mptcp_ccc {
	u64	alpha;
	bool	forced_update;
	u64 max_alpha;//Ph added
	u64 currts;//Ph added
};
...
//Ph added
static inline void mptcp_set_max_alpha(const struct sock *meta_sk, u64 max_alpha)
{
	((struct mptcp_ccc *)inet_csk_ca(meta_sk))->max_alpha = max_alpha;
}
//
static inline u64 mptcp_get_max_alpha(const struct sock *meta_sk)
{
	return ((struct mptcp_ccc *)inet_csk_ca(meta_sk))->max_alpha;
}
static inline void mptcp_set_currts(const struct sock *meta_sk, u64 currts)
{
	((struct mptcp_ccc *)inet_csk_ca(meta_sk))->currts = currts;
}
static inline u64 mptcp_get_currts(const struct sock *meta_sk)
{
	return ((struct mptcp_ccc *)inet_csk_ca(meta_sk))->currts;
}
//
...
static void mptcp_ccc_recalc_alpha(const struct sock *sk)
{
	...
	u64 max_alpha = 1, timediff = 0, currts = 0;
	int tcp_vmdetected = 0;

	if (!mpcb)
		return;

	//Ph added
	currts = (u64) ktime_to_ms(ktime_get_boottime());
	mptcp_for_each_sub(mpcb, mptcp) {
		const struct sock *sub_sk = mptcp_to_sock(mptcp);
		struct tcp_sock *sub_tp = tcp_sk(sub_sk);

		tcp_vmdetected += sub_tp->tcp_vmdetected;
	...
	mptcp_for_each_sub(mpcb, mptcp) {
		...
		//Ph added
		if (tcp_vmdetected > 0){
			sub_tp->srtt_us = sub_tp->tcp_prevsrtt;
		}
		//
	...
	if (unlikely(!alpha))
		alpha = 1;
	//Ph added
	max_alpha = mptcp_get_max_alpha(mptcp_meta_sk(sk));
	
	timediff = currts - mptcp_get_currts(mptcp_meta_sk(sk));
	if(timediff > diff_interval){
		max_alpha = alpha;
	}
	else{
	...
	if(ntohs( sk->sk_dport)==5001){
		printk("skno %u timediff %llu alpha %llu maxalpha %llu vmdetected %d", sk->sk_num, timediff, alpha, max_alpha, tcp_vmdetected);
	}
	//

exit:
	//Ph added
	mptcp_set_max_alpha(mptcp_meta_sk(sk), max_alpha); 
	mptcp_set_currts(mptcp_meta_sk(sk), currts);
	//
...
static void mptcp_ccc_init(struct sock *sk)
{
	...
	if (mptcp(tcp_sk(sk))) {
		...
		mptcp_set_max_alpha(mptcp_meta_sk(sk), 5);
		mptcp_set_currts(mptcp_meta_sk(sk), currts);
	}
